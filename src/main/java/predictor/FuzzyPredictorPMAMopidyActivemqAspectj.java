...
public class FuzzyPredictorPMAMopidyActivemqAspectj implements TagPredictor{
	...
	public int predictTagIndex(Instance item, Instances data, FeatureRequestOL request, int index) {
		...
		String[] uselessPattern1 = new String[] { "would like to work", "let me know","best wish" };
		...
		if(content.contains("Morphia too"))
			System.out.println("BUG");
			

		if ((numValidWords == 0 || numValidVerbs == 0) && !trimed && matchIsGOOD!=1) {
			result = 2;
		}
		...
		if (numValidWords < 2  && matchIsGOOD!=1) {
			result = 2;
		}
		...
		if(contentLower.startsWith("similar to"))
			result = 0;
		...
		boolean wouldbenice = contentLower.contains("would be nice ") ||contentLower.contains("would be preferable")
		...
		if (contentLower.startsWith("please") || contentLower.startsWith("the idea is")
				...
				|| contentLower.contains("we'd need") || contentLower.contains("OGM has to")|| contentLower.contains("this jira is to"))
				// subject=OGM

			result = 1;

		if (contentLower.contains("i like to")||contentLower.contains("i'd like to"))
			result = 1;
		if (subject.equalsIgnoreCase("candidate"))
			result = 0;
		if(contentLower.startsWith("for example"))
			result = 5;
		
		if(contentLower.startsWith("however")&&containNEG == 1)
			result = 4;
		
		if(contentLower.startsWith("however")&&contentLower.contains("only"))
			result = 4;
